...
def montly_update(url, page_val, master_pickle):
    book_list_layered = []

    for j in range(1,page_val+1):
        book_list_layered.append(scrape_list_page(url_begin+str(j)))

    book_list = [l[i] for l in list for i in range(0,len(l))]
    master = pickle.load(open(master_pickle, 'rb'))
    titles = []
    for book in master:
    ...
    pickle.dump(new_books, open('update.pickle', 'wb'))
